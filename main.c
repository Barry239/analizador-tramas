...
void analizaUDP(unsigned char t[], unsigned char ihl) {
    ...
    // Suma de control
    printf("Suma de control:\t\t0x%02x 0x%02x ", t[ihl + 20], t[ihl + 21]);
    pseudocabecera(t, ps);
    cs = checksum(ps, t, ihl + 14, length, ihl + 20);
    if (cs == ((t[ihl + 20] << 8) | t[ihl + 21])) printf("(Correcto)\n");
    else printf("(Incorrecto, 0x%02x 0x%02x)\n", cs >> 8, cs & 0xff);
